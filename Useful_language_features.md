...
12. [`12-nim-orthogonal-language-and-compiler-ideas.md`](docs/useful_language_features/12-nim-orthogonal-language-and-compiler-ideas.md) — notes on orthogonal low-level language design ideas inspired by Nim and how they map onto Contextlang.
...
18. [`18-current-surface-ergonomics.md`](docs/useful_language_features/18-current-surface-ergonomics.md) — implemented reference for current surface features such as default and named arguments, `..` forwarding, effect declarations, `signal`, local `can` grants, effects aliases, implicit contexts, `params` / `with args`, brace destructuring and updates, filtered loops, `do:` blocks, `defer`, index fallback, store/dict sugar, explicit `parallel for`, lambdas, char literals, and shorthand cast hooks.

...
| 3 | **Plain recursive `enum` ⇒ region-backed default** with storeless `new`/`match`. **Slice 0+0b DONE**: a plain recursive enum is promoted (syntactic self-reference at declare-time → `Packed`+`RecursivePlain`), the full zero-ceremony surface runs (plain enum + `common(...)` + **bare constructors** + storeless match → `eval(make(10))=1024`). **Remaining (Slice 2): AoS *storage*** — `packedEnumABIAoS` (one contiguous `{common,tag,payload}` record per node) + the parameterized index-handle ABI (Phase 2 width/sentinel/overflow) for the *speed*; today it's still SoA-backed. | 0, 1 |

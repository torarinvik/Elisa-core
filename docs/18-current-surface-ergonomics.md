...
- static functions must return on all paths; direct recursive static calls must visibly decrease a parameter with `parameter - positive_constant` and have a visible lower-bound base case such as `if n <= 0: return ...` / `if n > 0: recurse else return ...` for signed counters or `if n == 0: return ...` / `if n != 0: recurse else return ...` for unsigned counters; indirect recursive static cycles are rejected for now, and the evaluator reports a call-depth limit if a compile-time computation grows too large

...
- static functions must return on all paths; direct recursive static calls must visibly decrease a parameter with `parameter - positive_constant`, and the evaluator still keeps a depth limit as a backstop

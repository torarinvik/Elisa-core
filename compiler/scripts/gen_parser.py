#!/usr/bin/env python3
"""Generate parser.go for the llcontext compiler."""
import os

outpath = os.path.join(os.path.dirname(__file__), '..', 'parser', 'parser.go')

content = r'''package parser
...
func (p *Parser) parseExprOrAssignStmt() ast.Stmt {
	...
	if p.peek() == lexer.TOKEN_IDENT && p.pos+1 < len(p.tokens) && p.tokens[p.pos+1].Kind == lexer.TOKEN_COLON {
		// Could be var decl or field access followed by assignment
		// Var decl: ident COLON type ...
		name := p.cur().Text
		savedPos := p.pos
		p.advance() // ident
		p.advance() // colon
		...
		mutable := false
		// Check if next is a type or "mutable"
		if p.match(lexer.TOKEN_MUTABLE) {
			mutable = true
		}
		...
		case lexer.TOKEN_DOT:
			...
			if p.peek() == lexer.TOKEN_AMPERSAND {
				...
				if p.peek() == lexer.TOKEN_LPAREN {
					...
					target := &ast.RefType{
						Position: castPos,
						Elem:     &ast.NamedType{Position: castPos, Name: castName},
						Nullable: nullable,
					}
			...
			if p.peek() == lexer.TOKEN_LPAREN {
				// Disambiguate: is it a method call or a cast?
				// In llcontext, Type names start uppercase, func names start lowercase
				if len(field) > 0 && field[0] >= 'A' && field[0] <= 'Z' {
					// Cast: expr.Type()
					castPos := pos
					p.advance() // (
					p.expect(lexer.TOKEN_RPAREN)
...
print(f'Wrote parser to {outpath}')

...
func TestFactTransformsFromCFGFlowInstrsProjectsAllCoreInstructionKinds(t *testing.T) {
	...
	want := []FactTransform{
		{Kind: FactTransformConsume, Classes: []FactClass{FactUsage}, Target: "thread", Source: "control-flow instruction", Reason: "explicit move"},
		{Kind: FactTransformInvalidate, Classes: []FactClass{FactRegionDeps}, Target: "scratch", Source: "cp", Reason: "restore region checkpoint"},
		{Kind: FactTransformProduce, Classes: []FactClass{FactRepresentation, FactStorage}, Target: "first", Source: "scratch", Reason: "allocation produces value"},
		{Kind: FactTransformRebase, Classes: []FactClass{FactStoreDeps}, Target: "store", Source: "control-flow instruction", Reason: "freeze rebases store provenance"},
		{Kind: FactTransformRecompute, Classes: []FactClass{FactTypestate}, Target: "player.health", Source: "control-flow instruction", Reason: "assign"},
		{Kind: FactTransformRefine, Classes: []FactClass{FactAliasClass}, Target: "alias", Source: "first", Reason: "var init alias"},
	}
...
func TestFactTransformsFromCFGGuardsProjectsGuardFacts(t *testing.T) {
	...
	want := []FactTransform{
		{Kind: FactTransformRefine, Classes: []FactClass{FactRefState}, Target: "box", Source: "control-flow guard", Reason: "guard proves non-null"},
		{Kind: FactTransformRefine, Classes: []FactClass{FactTypestate}, Target: "node", Source: "control-flow guard", Reason: "guard proves variant Expr.Int"},
		{Kind: FactTransformRefine, Classes: []FactClass{FactOptimization}, Target: "start", Source: "control-flow guard", Reason: "guard proves <= end"},
	}
	if !reflect.DeepEqual(got, want) {
...
func TestFactTransformsFromFunctionSignatureProjectsPoststatesAndPermissions(t *testing.T) {
	...
	wantPoststates := []FactTransform{
		{Kind: FactTransformEnsure, Classes: []FactClass{FactTypestate}, Target: "player", Source: "ensures always", Reason: "ensures typestate Alive"},
		{Kind: FactTransformEnsure, Classes: []FactClass{FactTypestate, FactRefState}, Target: "player", Source: "ensures return false", Reason: "ensures preserve"},
		{Kind: FactTransformEnsure, Classes: []FactClass{FactRefState}, Target: "slot", Source: "ensures return true", Reason: "ensures refstate &"},
	}

...
// structurally shareable, so spawn1 must reject it (parity with the closure-capture
// path, which already rejects a captured darray header).
func TestSpawnRejectsBareDarrayDataArg(t *testing.T) {
	result := analyzeTreeTestSourceWithSemanticErrors(t, "thread_darray_arg.elisa", `def spawn1[A, R](fn: func(A) -> R, arg: A) -> Thread[R, Joinable]:
    return zeroed
def dworker(xs: darray[i64]) -> i64:
    return xs[0]
...
func TestSpawnRejectsGenericStructCaptureWithContainerField(t *testing.T) {
	const prelude = `def spawn1[A, R](fn: func(A) -> R, arg: A) -> Thread[R, Joinable]:
    return zeroed
`
	generic := analyzeTreeTestSourceWithSemanticErrors(t, "cap_generic.elisa", prelude+`struct Holder[T]:
    items: darray[T]
def start(h: Holder[i64]) -> Thread[i64, Joinable]:
    return spawn1(lambda (ignored: i64) => h.items[0], 0)
`)
	...
	nonGeneric := analyzeTreeTestSourceWithSemanticErrors(t, "cap_plain.elisa", prelude+`struct HolderC:
    items: darray[i64]
def start(h: HolderC) -> Thread[i64, Joinable]:
    return spawn1(lambda (ignored: i64) => h.items[0], 0)
`)

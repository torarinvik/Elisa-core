...
// A TERNARY element `[v if c else w]` selects an inner-region container at runtime; the chosen
// header is copied into the outer buffer and dangles. The element's region is invisible on the
// ternary's result type, so the interior-region walk must descend into both branches.
func TestNestedRegionTernaryElementRejected(t *testing.T) {
	res := analyzeTreeTestSourceWithSemanticErrors(t, "ternary_elem_escape.elisa", `def f(c: bool) -> i64:
    can Memory.Allocate, Abort.Panic:
        region outer(8192):
            outer_list: mutable darray[darray[u8]] @outer = []
            region a(4096):
                ...
                outer_list <- [v if c else w]
            return 0
`)
	if all := strings.Join(res.Errors(), "\n"); !strings.Contains(all, "longer-lived region \"outer\"") {
		t.Fatalf("expected ternary-selected inner-region element to be rejected; got: %s", all)
	}
}

// A STRUCT LITERAL `Holder(xs: v)` with an inner-region field, assigned into a region-outliving
// local, copies the dangling field. Struct construction is not a field-store, so the interior-taint
// table never tainted it; the interior-region walk over the literal's args catches it.
func TestNestedRegionStructLiteralFieldRejected(t *testing.T) {
	res := analyzeTreeTestSourceWithSemanticErrors(t, "structlit_field_escape.elisa", `struct Holder:
    xs: mutable darray[u8]
...
`)
	if all := strings.Join(res.Errors(), "\n"); !strings.Contains(all, "outlives the region") {
		t.Fatalf("expected struct-literal inner-region field store into outliving local to be rejected; got: %s", all)
	}
}

// PUSHING a fresh list literal `ol.push([v])` whose element is an inner-region container: the push
// element-check sees only the literal's own (outer) region, so the interior of the pushed producer
// must be checked too.
func TestNestedRegionPushFreshListLiteralRejected(t *testing.T) {
	res := analyzeTreeTestSourceWithSemanticErrors(t, "push_freshlist_escape.elisa", `def f() -> i64:
    can Memory.Allocate, Abort.Panic:
        region outer(8192):
            outer_list: mutable darray[darray[darray[u8]]] @outer = []
            region a(4096):
                v: mutable darray[u8] @a = []
                v.push(66)
                outer_list.push([v])
...
`)
	if all := strings.Join(res.Errors(), "\n"); !strings.Contains(all, "longer-lived region \"outer\"") {
		t.Fatalf("expected push of a fresh list literal carrying an inner-region element to be rejected; got: %s", all)
	}
}

// A fresh list literal can also launder a region-less struct whose field was tainted by an earlier
// inner-region assignment. The interior walk's leaf case must consult the taint table, not just the
// expression type's own region stamp.
func TestNestedRegionFreshListWithTaintedStructElementRejected(t *testing.T) {
	res := analyzeTreeTestSourceWithSemanticErrors(t, "freshlist_tainted_struct_escape.elisa", `struct Holder:
    xs: mutable darray[u8]

def f() -> i64:
    can Memory.Allocate, Abort.Panic:
        region outer(8192):
            holders: mutable darray[Holder] @outer = []
            h: mutable Holder = zeroed
            region a(4096):
                v: mutable darray[u8] @a = []
                v.push(66)
...
`)
	if all := strings.Join(res.Errors(), "\n"); !strings.Contains(all, "longer-lived region \"outer\"") {
		t.Fatalf("expected fresh list carrying a tainted struct element to be rejected; got: %s", all)
	}
}

// Positive control: a struct literal whose field shares the (inside-block) target's lifetime is sound
// and must stay accepted.
func TestNestedRegionStructLiteralSameLifetimeAccepted(t *testing.T) {
	res := analyzeTreeTestSourceWithSemanticErrors(t, "structlit_same_ok.elisa", `struct Holder:
    xs: mutable darray[u8]
...
def f() -> i64:
    can Memory.Allocate, Abort.Panic:
        region a(8192):
            ...
            return i64(h.xs[0])
`)
	if all := strings.Join(res.Errors(), "\n"); strings.Contains(all, "dangling") || strings.Contains(all, "outlives") {

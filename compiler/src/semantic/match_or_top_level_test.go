...
func TestTopLevelEnumOrPatternSharesCompatibleBindings(t *testing.T) {
...
`)
	if errs := result.Errors(); len(errs) != 0 {
		t.Fatalf("alternatives with the same typed binding should expose it to the arm body, got: %v", errs)
	}
}

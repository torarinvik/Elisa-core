		...
		case reflect.Pointer:
			...
			if call, ok := v.Interface().(*ast.CallExpr); ok {
				if growth, ok := call.Func.(*ast.FieldExpr); ok && growth != nil && containerGrowthMethods[growth.Field] {
					...
					// `param.a.b.push(..)`, `param.f[i].push(..)`, etc. (any depth). A path deeper than
					// the bare param means the grown container is a (possibly nested) field of the
					// struct param, so the param must carry the region the field backing grows into.
					// (The bare-param case `param.push` is a CONTAINER param, handled by
					// paramContainerIsGrown / regionlessRefContainer, not here.)

...
func (a *Analyzer) resolveBuiltinSurfaceType(expr *ast.BuiltinTypeExpr) Type {
	...
	case "darray":
		...
		if len(expr.ValueArgs) == 0 {
			return &DArrayType{Elem: a.resolveType(expr.TypeArgs[0]), ElemTypeExpr: expr.TypeArgs[0], Shape: &WildcardShape{}, SurfaceName: "darray", Region: expr.Region}
		}
		return &DArrayType{Elem: a.resolveType(expr.TypeArgs[0]), ElemTypeExpr: expr.TypeArgs[0], Shape: a.resolveShapeExpr(expr.ValueArgs[0]), SurfaceName: "darray", Region: expr.Region}
	case "dict":

...
func (s *functionState) emitOpenStmt(stmt *ast.OpenStmt) error {
	...
	enumType, ok := resolveMatchableEnumType(actualType)
	if ok {
		storeBinding, err := s.resolvePackedMatchStoreBinding(enumType, stmt.Value, stmt.Store)
		if err != nil {
			return err
	...
	treeType, _, ok := resolveMatchableTreeCategoryTypeBackend(actualType)
	if ok {
		return s.emitTreeOpenStmt(stmt, actualType, treeType)
	}
	return fmt.Errorf("open requires a packed enum or tree-category value")
}

func (s *functionState) emitTreeOpenStmt(stmt *ast.OpenStmt, actualType semantic.Type, treeType *semantic.TreeCategoryType) error {
	if stmt == nil || stmt.Pattern == nil {
		return nil
	}
	if treeType == nil {
		return fmt.Errorf("open requires a tree-category value")
	}
	if stmt.Store != nil {
		return fmt.Errorf("tree open over %q does not take an in-store clause", treeType.Name)
	}
	...
	matchPattern := &ast.MatchVariantPattern{Position: stmt.Pattern.Position, EnumName: stmt.Pattern.EnumName, Variant: stmt.Pattern.Variant, Args: append([]ast.MatchPatternArg(nil), stmt.Pattern.Args...)}
	s.pushScope()
	if _, _, err := s.emitMatchPatternTest(matchPattern, treeValue, nil, actualType, nil, stmt.Value, nil, successBB, failBB); err != nil {
		s.popScope()
		return err
	}
	C.LLVMPositionBuilderAtEnd(s.builder, successBB)
...
func (s *functionState) emitViewStmt(stmt *ast.ViewStmt) error {
	...
	treeType, _, ok := resolveMatchableTreeCategoryTypeBackend(actualType)
	if ok {
		return s.emitTreeViewStmt(stmt, actualType, treeType)
	}
	return fmt.Errorf("view requires a packed enum or tree-category value")
}

func (s *functionState) emitTreeViewStmt(stmt *ast.ViewStmt, actualType semantic.Type, treeType *semantic.TreeCategoryType) error {
	if stmt == nil || stmt.Pattern == nil {
		return nil
	}
	if treeType == nil {
		return fmt.Errorf("view requires a tree-category value")
	}
	if stmt.Store != nil {
		return fmt.Errorf("tree view over %q does not take an in-store clause", treeType.Name)
	}
	variant, ok := treeType.Variant(stmt.Pattern.Variant)
	if !ok {
		return fmt.Errorf("tree category %s has no variant %s", treeType.Name, stmt.Pattern.Variant)
	}
	viewType := treeType.VariantViewType(variant)
	treeValue, _, err := s.emitExpr(stmt.Value, nil)
	if err != nil {
		return err
	}
	...
	C.LLVMPositionBuilderAtEnd(s.builder, successBB)
	if stmt.Pattern.Name != "" && stmt.Pattern.Name != "_" {
		if err := s.emitMoveBindLocal(stmt.Pattern.Name, viewType, treeValue); err != nil {
			s.popScope()
			return err
		}
	}

...
`)
	...
	zDecl, ok := fn.Body[2].(*ast.VarDeclStmt)
	if !ok {
		t.Fatalf("expected third local declaration, got %T", fn.Body[2])
	}
	...
	exprStmt, ok := fn.Body[3].(*ast.ExprStmt)
	if !ok {
		t.Fatalf("expected expression statement, got %T", fn.Body[3])
	}

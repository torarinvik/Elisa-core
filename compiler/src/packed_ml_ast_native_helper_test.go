...
func buildCachedNativeArtifacts(tb testing.TB, repoRoot string, spec nativeArtifactSpec) nativeArtifacts {
	...
	entry.once.Do(func() {
		...
		compileArgs = append(compileArgs, buildArtifacts.object, profilerFallbackPath, "-o", buildArtifacts.executable)
...
func packedMLASTMegaNativeSpec(repoRoot string, objectOpt string) nativeArtifactSpec {
	return nativeArtifactSpec{
		...
		shimPaths:       []string{filepath.Join(repoRoot, "Code", "benchmarks", "json_parser_runtime_shims.c")},
		generateHeader:  true,

...
func printDecl(w io.Writer, d ast.Decl, level int) {
	...
	case *ast.StructDecl:
		...
		tparams := formatFuncGenericParams(n.GenericParams, n.TypeParams, regionParams, nil)
		if n.RegionOwner != "" {
	...
	case *ast.FuncDecl:
		...
		tparams := formatFuncGenericParams(n.GenericParams, n.TypeParams, n.RegionParams, n.PermissionParams)
		ret := ""
		if n.ReturnType != nil {
...
func typeStr(t ast.TypeExpr) string {
	...
	case *ast.FuncTypeExpr:
		...
		return "func(" + strings.Join(parts, ", ") + ")" + withClause + ret + formatPermissionRefs(n.Permissions)

...
func generatedNativeCallbackRuntimeShimSource(result *semantic.Result) string {
	...
	b.WriteString("#if defined(_WIN32)\n")
	b.WriteString("  uintptr_t handle = _beginthreadex(NULL, 0, elisa_native_u32_thread_main, &start, 0, NULL);\n")
	b.WriteString("  if (handle == 0) { return fallback; }\n")
	...
	b.WriteString("#if defined(_WIN32)\n")
	b.WriteString("  uintptr_t handle = _beginthreadex(NULL, 0, elisa_native_u32_context_thread_main, raw, 0, NULL);\n")
	b.WriteString("  if (handle == 0) { return -1; }\n")
	...
	b.WriteString("  elisa_native_u32_callback_context *ctx = (elisa_native_u32_callback_context *)raw;\n")
	b.WriteString("  if (handle == 0 || ctx == NULL) { return fallback; }\n")
	b.WriteString("#if defined(_WIN32)\n")
	b.WriteString("  if (WaitForSingleObject((HANDLE)handle, INFINITE) != WAIT_OBJECT_0) { CloseHandle((HANDLE)handle); return fallback; }\n")
	b.WriteString("  CloseHandle((HANDLE)handle);\n")
	b.WriteString("#else\n")
	...
	b.WriteString("#if defined(_WIN32)\n")
	b.WriteString("  uintptr_t handle = _beginthreadex(NULL, 0, elisa_native_u32_context_thread_main, ctx, 0, NULL);\n")
	b.WriteString("  if (handle == 0) { return fallback; }\n")
	b.WriteString("  if (WaitForSingleObject((HANDLE)handle, INFINITE) != WAIT_OBJECT_0) { CloseHandle((HANDLE)handle); return fallback; }\n")
	b.WriteString("  CloseHandle((HANDLE)handle);\n")
	b.WriteString("#else\n")

...
int main(int argc, char **argv) {
    ...
    if (lua_frontend_execute_status_with_len(input, (uintptr_t)input_len) != 0) {
        fprintf(stderr, "execute frontend rejected %s\n", path);
        free(input);
        return 1;
    }

    int64_t warmup_checksum = lua_frontend_execute_checksum_with_len(input, (uintptr_t)input_len);

    struct timespec start = {0};
    struct timespec end = {0};
    clock_gettime(CLOCK_MONOTONIC, &start);

    int64_t checksum_acc = lua_frontend_execute_repeated_checksum_with_len(input, (uintptr_t)input_len, (uintptr_t)iterations);

...
int main(int argc, char** argv) {
    ...
    int64_t warmup_checksum = json_parser_ast_checksum(input);
    if (warmup_checksum < 0) {
        fprintf(stderr, "ast parser rejected %s\n", path);
        free(input);
        return 1;
    }
    ...
    for (int i = 0; i < iterations; ++i) {
        checksum_acc += json_parser_ast_checksum(input);
    }
    ...
    printf("mode=ast file=%s bytes=%zu iterations=%d checksum=%" PRId64 " total_checksum=%" PRId64 " seconds=%.6f MiB/s=%.2f\n",
           path,
           input_len,
           iterations,
           warmup_checksum,
